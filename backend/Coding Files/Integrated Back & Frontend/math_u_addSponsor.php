		...
		if ($spr_id != 0){
			...
			if (UpdateID($new_id, $spr_id, $mysqli)===TRUE){
				...
				if (AddToUsers($email, $new_id, $mysqli)===TRUE){
					...
					echo json_encode("Could not add Account to AllUsers");
				}
				//-------------------------------------------------
			}
			else {
				echo json_encode("Could not update sponsor ID!");
			}
			//---------------------------------------------------------
			
		}
		else{
			echo json_encode("Could not Add Admin!");
		}
		//-------------------------------------------------------------------------	
	}
	else{
		echo json_encode("Could not Insert New Company Try Again!");
	}
}
else{
	echo json_encode("Email not unique!");
}	
